...
- **WHEN** the `lv-bootstrap` skill/command or `lv start` runs for a feature ID whose `docs/features/<feature-id>/` directory already exists locally, but the configured Lark Features table has no record for that feature ID
...
- **WHEN** the `lv-bootstrap` skill/command runs for a feature ID that already has a record in the Lark Features table
