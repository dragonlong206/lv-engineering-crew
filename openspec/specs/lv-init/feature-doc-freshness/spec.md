...
The system SHALL configure the installed OpenSpec archive workflow so that, for each feature ID recorded against the change being archived, its guidance instructs regenerating that feature's docs before the archive completes.
...
- **WHEN** an engineer archives a change whose recorded context lists no feature IDs
