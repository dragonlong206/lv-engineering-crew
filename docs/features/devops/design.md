...
- First publish is necessarily manual, not CI-driven — npm's Trusted Publisher entry is configured from the package's own settings page on npmjs.com, which doesn't exist until the package has been published at least once, and `lv-engineer-crew` has never been published. This wasn't caught during the original design pass (which assumed only that no `NPM_TOKEN` secret existed yet, not that the package itself didn't exist) and was corrected afterward: `README.md` now documents the one-time manual `npm publish` a maintainer must run before the Trusted Publisher entry — and therefore this workflow — can work at all.
