...
- Presentation layer: `printStateSummary(changeId, state)` renders the summary.

The command does not introduce its own state machine or caching. It is entirely driven by the branch name and the on-disk change state.
...
`lv status` only consumes this schema. There are no step records, progress counters, or approval timestamps in the current model.
...
- Output is intentionally a human-readable summary of persisted change context, not a workflow dashboard. The code prints stable metadata only, and leaves step tracking out of the current design.
