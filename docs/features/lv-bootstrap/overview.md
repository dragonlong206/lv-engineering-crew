...
`lv-bootstrap` generates feature documentation from code. It has two supported modes: a path-based mode that documents one or more user-supplied files or directories, and an autonomous scan mode that explores the repository and refines existing draft docs when they are present.
...
- **Path-based doc generation**: `bootstrapAgent` in `src/cli/bootstrap.ts` generates Markdown from a prebuilt code context string.
- **Autonomous scan agent**: `createBootstrapScanAgent()` in `src/agents/bootstrap-agent.ts` creates a Mastra `Agent` with `listFiles`, `readFile`, and `searchCode` tools.
