...
`lv-bootstrap` generates feature documentation from the actual repository code. It supports two entry paths: a path-based mode that documents user-supplied files or directories, and an autonomous scan mode that explores the repo with tools and can refine any existing draft docs.

## Main Components
- **CLI command**: `bootstrap <feature-id>` is registered in `src/index.ts`.
- **Bootstrap runner**: `runBootstrap()` in `src/cli/bootstrap.ts` chooses path-based generation or autonomous scan mode.
- **Path-based generation agent**: an inline Mastra `Agent` named `lv-bootstrap-agent` generates the two markdown files from a prebuilt code context string.
...
8. After generation, the command may also sync a new feature to Lark when that integration is enabled in config and the required credentials are available.
