...
- **Prompt layer**: `src/prompts.ts` defines the user-facing prompts and the document templates used by all bootstrap modes.
...
- `runBootstrap(featureId: string, pathsArg?: string, hint?: BootstrapScanHint, opts?: BootstrapOpts): Promise<void>`
  ...
  - Routes to path-based generation when `pathsArg` is provided.
  - Routes to autonomous scan mode otherwise.
